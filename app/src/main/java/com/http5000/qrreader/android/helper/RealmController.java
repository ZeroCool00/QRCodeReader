...
public class RealmController {
    ...
    public RealmResults<Model> getBooks() {
 
        return realm.where(Model.class).findAll();
    }
 
    //query a single item with the given id
    public Model getBook(String id) {
    ...
    public boolean hasBooks() {
        ...
        return !realm.allObjects(Model.class).isEmpty();
    }
 
    //query example
    public RealmResults<Model> queryedBooks() {
 
        return realm.where(Model.class)
                .contains("qrText", "qrText 0")
                .or()
                .contains("id", "Realm")
                .findAll();
